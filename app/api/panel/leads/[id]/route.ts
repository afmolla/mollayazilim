...
import { updateLeadMeta, type LeadStatus } from "@/lib/lead-store";

type Body = { status?: unknown; note?: unknown };

function parseLeadStatus(raw: string): LeadStatus | undefined {
  if (raw === "yeni" || raw === "aranacak" || raw === "kapandi") return raw;
  return undefined;
}
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  return withSiteFromRequest(req, async () => {
    ...
    const next = await updateLeadMeta(id, {
      status: parseLeadStatus(statusRaw),
      note,
    });
