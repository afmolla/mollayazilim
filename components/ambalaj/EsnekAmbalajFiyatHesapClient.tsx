  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 md:px-6 md:py-14">
      <div className="max-w-3xl">
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[var(--brand)]">Online teklif</p>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-6 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Kalınlık (mikron µm)" hint="Tipik: 20–80 µm gıda, 60–120 µm ağır duty">
              <input
                type="number"
                min={15}
                max={200}
            ...
            {g.urunTipi === "torba" ? (
              ...
              <Field label="Metraj (metre)" hint="Bobin uzunluğu — otomatik dolum hatları için">
                <input
                  type="number"
                  min={100}
                  step={100}
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-4">
            <label className="flex cursor-pointer items-center gap-3">
              ...
              <span className="font-medium text-[var(--text)]">Baskı istiyorum (flexo / rotogravür)</span>
            </label>
            {g.baski ? (
              <div className="mt-4">
                <Field label="Baskı renk sayısı">
                  <input
                    type="number"
                    min={1}
                    max={8}
        ...
        <div className="space-y-4 lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-2xl border border-[var(--brand)]/30 bg-[var(--surface)] p-6 shadow-lg">
            <p className="text-sm font-semibold text-[var(--muted)]">Tahmini teklif aralığı (KDV hariç)</p>
            <p className="mt-2 text-3xl font-bold tabular-nums text-[var(--brand)]">
              {fmt(sonuc.toplamMin)} – {fmt(sonuc.toplamMax)} ₺
            </p>
            <dl className="mt-6 space-y-2 text-sm">
              <div className="flex justify-between gap-4">
                <dt className="text-[var(--muted)]">Tahmini ağırlık</dt>
